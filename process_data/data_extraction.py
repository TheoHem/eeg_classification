...
def extract_data_from_subject_viz(root_dir: str, n_s: int, datatype: str) -> tuple:
    """
    ...
    for n_b in n_b_arr:
        ...
        if datatype == "eeg":
            ...
            X = mne.read_epochs(file_name, verbose='WARNING')
            data[n_b] = X  # Keep MNE object for visualization
    ...
    # Stack NumPy data for other use cases
    X_stacked = np.vstack([data[b]._data for b in n_b_arr])
    Y_stacked = np.vstack([y[b] for b in n_b_arr])

    return data, X_stacked, Y_stacked
